...
module Helm.Toolchain.Frontend.Pipeline.Driver (
    pipeline
) where
...
pipeline :: [Path.Path Path.Abs Path.Dir] -> IO (Either Text Text)
pipeline paths = do
    helmFiles <- Crawl.collectHelmFiles paths
    (errors, nodes) <- Either.partitionEithers <$> M.mapM Extract.parseHeader helmFiles
    
    if not (null errors) then
        return $ Left $ Text.unlines $ map (Text.pack . PP.prettyShow) errors
    else
        case Ordering.buildOrdering nodes of
            Left err ->
                return $ Left $ Text.unlines $ map (Text.pack . PP.prettyShow) errors
            Right buildOrder ->
                Compile.compile' buildOrder
