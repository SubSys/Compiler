...
module Helm.Toolchain.Frontend.Pipeline.Data.Node (
    FilePath
  , Namespace(..)
  , Node(..)
  , toText
...
type FilePath = Path.Path Path.Abs Path.File



newtype Namespace = Namespace [Text]
    deriving (Show, Eq, Ord, Data.Typeable, Data.Data)
...
toText :: Namespace -> Text
toText (Namespace segs) = Text.intercalate (Text.pack ".") segs

toTexts :: [Namespace] -> [Text]
toTexts = map toText

fromText :: Text -> Namespace
fromText (breakOn -> segs) = Namespace segs
