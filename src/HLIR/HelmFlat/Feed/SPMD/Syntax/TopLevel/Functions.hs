...
module HLIR.HelmFlat.Feed.SPMD.Syntax.TopLevel.Functions (
    dropFunction
) where
...
import qualified HLIR.HelmFlat.Feed.SPMD.Syntax.BlockLevel.Stmt as S
-- *


dropFunction :: H.Decl.Function -> S.Decl.Function
dropFunction (H.Decl.Function (H.Etc.Binder_ name) args body (Just scheme@(H.T.Forall [] ty))) =
    S.Decl.Function
        (T.dropType output)
        (ID.dropIdent name)
        (map binder2Input args)
        (S.S.Block [S.dropExpr body])
    
    where
        output = Type.getReturnType ty


binder2Input :: H.Etc.Binder -> S.Etc.Input
