...
module HLIR.HelmFlat.Feed.SPMD.Driver (
    toSPMD
  , toSPMD'
) where
...
-- + SPMD AST Interface
import qualified LLIR.SPMD.Data.Interface   as SPMD
...
-- + Local
import qualified HLIR.HelmFlat.Feed.SPMD.Syntax.TopLevel.Functions as Decl
-- *


toSPMD :: IO (Either Text HelmFlat.Program) -> IO (Either Text SPMD.Program)
toSPMD upstream = do
    result <- upstream
...
toSPMD' :: HelmFlat.Program -> SPMD.Program
toSPMD' payload =
