...
module HLIR.HelmFlat.Feed.HelmLL.Syntax.Base.Types (
    dropScheme
  , dropType
) where
...
dropScheme :: HL.T.Scheme -> LL.T.Scheme
dropScheme (HL.T.Forall as ty) =
    LL.T.Forall
        (map ID.dropIdent as)
        (dropType ty)


dropType :: HL.T.Type -> LL.T.Type

dropType (HL.T.Tuple ts) =
    LL.T.Tuple (map dropType ts)

dropType (HL.T.List ty) =
    LL.T.List (dropType ty)

dropType (HL.T.Union name args) =
    LL.T.Union
        (ID.dropIdent name)
        (map dropType args)

dropType (HL.T.Var id') =
    LL.T.Var (ID.dropIdent id')

dropType (HL.T.Arr ty1 ty2) =
    LL.T.Arr
        (dropType ty1)
        (dropType ty2)

dropType HL.T.String =
    LL.T.String
