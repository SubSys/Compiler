...
module HLIR.HelmFlat.Feed.HelmLL.Syntax.Base.Ident (
    dropIdent
  , dropNamespace
) where
...
dropIdent :: HL.ID.Ident -> LL.ID.Ident
dropIdent (HL.ID.Ident txt ns) =
    LL.ID.Ident txt (dropNamespace ns)


dropNamespace :: Maybe HL.ID.Namespace -> Maybe LL.ID.Namespace
dropNamespace Nothing = Nothing
dropNamespace (Just (HL.ID.Namespace segs)) =
    Just $ LL.ID.Namespace segs
