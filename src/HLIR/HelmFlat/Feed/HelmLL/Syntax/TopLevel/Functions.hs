...
module HLIR.HelmFlat.Feed.HelmLL.Syntax.TopLevel.Functions (
    dropFunction
) where
...
import qualified HLIR.HelmFlat.Feed.HelmLL.Syntax.TermLevel.Stmt     as S
-- *




dropFunction :: HL.Decl.Function -> LL.Decl.Function
dropFunction (HL.Decl.Function name args expr scheme) =
    LL.Decl.Function
        (Etc.dropBinder name)
        (map Etc.dropBinder args)
        (LL.S.Block $ singleton $ S.dropExpr expr)
        (Core.applyMaybe T.dropScheme scheme)
