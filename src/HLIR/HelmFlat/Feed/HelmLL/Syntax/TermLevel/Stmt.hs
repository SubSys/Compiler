...
module HLIR.HelmFlat.Feed.HelmLL.Syntax.TermLevel.Stmt (
    dropExpr
) where
...
import qualified HLIR.HelmFlat.Feed.HelmLL.Syntax.TermLevel.Patterns as P
-- *
...
dropExpr :: HL.E.Expr -> LL.S.Stmt


dropExpr (HL.E.Lit val) =
    LL.S.Lit
        (Lit.dropLiteral val)

dropExpr (HL.E.Tuple items) =
    LL.S.Tuple
        (map dropExpr items)

dropExpr (HL.E.List xs) =
    LL.S.List
        (map dropExpr xs)

dropExpr (HL.E.Case con alts) =
    LL.S.Case
        (dropExpr con)
        (map (P.dropCaseAlt dropExpr) alts)

dropExpr (HL.E.FunCall name args) =
    LL.S.FunCall
        (ID.dropIdent name)
        (map dropExpr args)

dropExpr (HL.E.ConCall name args) =
    LL.S.ConCall
        (ID.dropIdent name)
        (map dropExpr args)

dropExpr (HL.E.Ref name) =
    LL.S.Ref
        (ID.dropIdent name)
