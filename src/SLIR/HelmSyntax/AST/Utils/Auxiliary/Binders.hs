...
module SLIR.HelmSyntax.AST.Utils.Auxiliary.Binders (
    voidBinderTypes
  , freshBinder
) where
...
voidBinderTypes :: (Data a, Typeable a) => a -> a
voidBinderTypes = Uni.transformBi f
    where
        f :: Etc.Binder -> Etc.Binder
        f (Etc.Binder ident _) = Etc.Binder ident Nothing
...
freshBinder idx =
    ...
    where
        ...
        newBinder :: Etc.Binder
        newBinder =
            Etc.Binder_ (ID.Ident_ name)
