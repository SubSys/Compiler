...
module SLIR.HelmSyntax.AST.Utils.Auxiliary.Functions.Recursive (
    isRecDecl
) where
...
isRecDecl (Decl.Function (Etc.Binder name _) _ body _ _) =
    let
        x1 = [y | (E.Var y _) <- Uni.universe body]
        x2 = [y | (E.FunCall_ y _) <- Uni.universe body]
    in
