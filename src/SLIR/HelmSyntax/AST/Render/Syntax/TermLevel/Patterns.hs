...
module SLIR.HelmSyntax.AST.Render.Syntax.TermLevel.Patterns (
    renderCaseAlt
  , renderPattern
) where
...
renderPattern (P.Var name meta) = Etc.renderBinder name
...
renderPattern (P.ListCons xs Nothing meta) =
...
renderPattern (P.ListCons xs (Just rest) meta) =
    let xs' = map renderPattern xs
            |> Util.punctuate "::"
            |> Util.hcat
...
renderPattern (P.Constr name args meta) =
