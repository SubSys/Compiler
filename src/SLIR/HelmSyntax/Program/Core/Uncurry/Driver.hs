...
module SLIR.HelmSyntax.Program.Core.Uncurry.Driver (
    uncurryTerms
  , uncurryTerms'
) where
...
-- + Local
import qualified SLIR.HelmSyntax.Program.Core.Uncurry.Syntax as Syntax
-- *
...
uncurryTerms :: IO (Either Text I.Program) -> IO (Either Text I.Program)
uncurryTerms upstream = do
    result <- upstream
    ...
    case result of
        ...
        Right payload ->
            return
                $ Right
                $ uncurryTerms' $ Driver.typeCheck' payload


uncurryTerms' :: I.Program -> I.Program
uncurryTerms' payload@(I.getFunctions -> decls) =
    payload |> I.updateFunctions (Syntax.uncurryExpr $ Driver.globalize decls)
