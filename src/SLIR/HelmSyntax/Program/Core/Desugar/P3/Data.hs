...
module SLIR.HelmSyntax.Program.Core.Desugar.P3.Data (
    Overloads
  , IndexedOverloads
) where
...
--
type Overloads        = Map.Map ID.Ident [Decl.Function]
type IndexedOverloads = Map.Map ID.Ident (Map.Map T.Type Decl.Function)

type OldIdent = ID.Ident
type NewIdent = ID.Ident
