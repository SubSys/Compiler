...
module SLIR.HelmSyntax.Program.Core.Desugar.P2.Pass (
    desugarP2
) where
...
import qualified SLIR.HelmSyntax.AST.Utils.Auxiliary.Functions.Inline    as Decl
...
desugarP2 :: I.Program -> I.Program
desugarP2 payload =
    payload |> Driver.index'
            |> pipeline
            |> Driver.typeCheck'
...
pipeline payload@(I.getFunctions -> decls) =
    let
        (supers, rest) = SP.splitSuperposedDecls decls
    in
        I.updateFunctions' payload (Uni.transformBi (exprScheme supers) rest)


exprScheme :: Map.Map ID.Ident Decl.Function -> E.Expr -> E.Expr
exprScheme supers expr@(E.FunCall ident args ty meta) =
    error $ Text.unpack $ Text.unlines
        [ Text.pack "SLIR.HelmSyntax.Program.Core.Desugar.P2.Pass.desugarP2"
        , Text.pack "Don't **uncurry applications** until after this pass!"
        ]

exprScheme supers expr@(E.Var ident _)
    | Just (Decl.inlineDecl -> term) <- Map.lookup ident supers =
        term




exprScheme supers e = e
