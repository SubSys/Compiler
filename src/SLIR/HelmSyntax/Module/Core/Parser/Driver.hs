...
module SLIR.HelmSyntax.Module.Core.Parser.Driver (
    runModuleParser
  , runHeaderParser
) where
...
-- + Frameworks
import qualified Framework.Text.Parser as Parser (parseErrorPretty, runParser)
...
-- + Local
import qualified SLIR.HelmSyntax.Module.Core.Parser.Syntax as Syntax
-- *
...
--
runModuleParser :: Pre.FilePath -> IO String -> IO (Either Text I.Module)
runModuleParser path source = do
    result <- Parser.runParser (Syntax.parseModule path) path <$> source
...
--
runHeaderParser :: Pre.FilePath -> IO String -> IO (Either Text I.Header)
runHeaderParser path source = do
    result <- Parser.runParser (Syntax.parseHeader path) path <$> source
    case result of
        Left err ->
            return $ Left $ Text.pack (Parser.parseErrorPretty err)
        Right payload ->
