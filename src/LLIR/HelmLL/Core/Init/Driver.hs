...
module LLIR.HelmLL.Core.Init.Driver (
    init
  , init'
) where
...
-- + Local
import qualified LLIR.HelmLL.Core.Init.Pass.BoolCases as Pass
-- *
...
pipeline :: [Decl.Function] -> [Decl.Function]
pipeline decls =
    decls |> Pass.boolCases2IFs
