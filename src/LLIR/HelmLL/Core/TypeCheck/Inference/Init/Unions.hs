...
module LLIR.HelmLL.Core.TypeCheck.Inference.Init.Unions (
    genUnionSigs
) where
...
genUnionSigs :: Decl.Union -> [(ID.Ident, T.Scheme)]
genUnionSigs union@(Decl.Union _ _ cs) =
    map (genConstrBinderSig union) cs



genConstrBinderSig :: Decl.Union -> Decl.Constr -> (ID.Ident, T.Scheme)
genConstrBinderSig decl@(Decl.Union unionName as _) (Decl.Constr conName args) =
...
union2Type (Decl.Union unionName as _) =
    T.Union unionName (map T.Var as)
