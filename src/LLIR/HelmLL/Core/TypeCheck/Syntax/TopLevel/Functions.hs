...
module LLIR.HelmLL.Core.TypeCheck.Syntax.TopLevel.Functions (
    inferDecl
) where
...
-- ++ Sub Infers
import qualified LLIR.HelmLL.Core.TypeCheck.Syntax.TermLevel.Expr as E
...
inferDecl fn@(SudoFFI.isSudoFFI' -> Just (Decl.Function name args expr (Just scheme))) = do
...
inferDecl fn@(Fn.isRecFunction -> True) = inferRecDecl fn
...
inferDecl (Decl.Function name args expr sig) = do
    ...
    (expr', et, _)       <- Scope.withLocalEnv exprEnv (E.inferBlock expr)
    ...
    -- *
    binder (Decl.Function name args' expr' sig) tv newEnv
...
inferRecDecl (Decl.Function name args expr sig) = do
    ...
    (expr', et, _)   <- Scope.withLocalEnv env (E.inferBlock expr)
    -- *
