...
module LLIR.HelmLL.Core.Index.Scope.Referable (
    referable
) where
...
referable :: ID.Ident -> Sys.State (ID.Ident, Sys.Subst)
referable ident = do
    s <- M.ask

    case Map.lookup ident s of
        Nothing    -> return (ident, Map.empty)
        Just newIdent -> return (newIdent, Map.empty)
