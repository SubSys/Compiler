...
module LLIR.HelmLL.Feed.GLSL.Driver (
    toGLSL
  , toGLSL'
) where
...
-- + Local
import qualified LLIR.HelmLL.Feed.GLSL.Syntax.TopLevel.Functions as Decl
-- *




toGLSL :: IO (Either Text I.Program) -> IO (Either Text GLSL.Program)
toGLSL upstream = do
    result <- upstream
