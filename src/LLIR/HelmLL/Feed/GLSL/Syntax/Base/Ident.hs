...
module LLIR.HelmLL.Feed.GLSL.Syntax.Base.Ident (
    dropIdent
  , dropNamespace
) where
...
dropIdent :: LL.ID.Ident -> CG.ID.Ident
dropIdent (LL.ID.Ident label ns) = CG.ID.Ident label (dropNamespace ns)

dropNamespace :: Maybe LL.ID.Namespace -> Maybe CG.ID.Namespace
dropNamespace Nothing = Nothing
dropNamespace (Just (LL.ID.Namespace segs)) = Just $ CG.ID.Namespace segs
