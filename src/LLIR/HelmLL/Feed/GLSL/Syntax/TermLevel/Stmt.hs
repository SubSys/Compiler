...
module LLIR.HelmLL.Feed.GLSL.Syntax.TermLevel.Stmt (
    dropBlock
  , dropStmt
) where
...
import qualified LLIR.HelmLL.AST.Tailored.GLSL.TermLevel.Stmt as LL.S
...
import qualified CGIR.GLSL.AST.Aliases.TermLevel.Stmt          as CG.S
...
import qualified LLIR.HelmLL.Feed.GLSL.Syntax.Base.Literals as Lit
-- *



dropBlock :: LL.S.Block -> CG.S.Block
dropBlock (LL.S.Block stmts) =
    CG.S.Block (map dropStmt stmts)


dropStmt :: LL.S.Stmt -> CG.S.Stmt
dropStmt (LL.S.Vec2Float x1 x2)       = CG.S.Vec2Float x1 x2
dropStmt (LL.S.Vec3Float x1 x2 x3)    = CG.S.Vec3Float x1 x2 x3
dropStmt (LL.S.Vec4Float x1 x2 x3 x4) = CG.S.Vec4Float x1 x2 x3 x4
dropStmt (LL.S.Vec2Int x1 x2)         = CG.S.Vec2Int x1 x2
dropStmt (LL.S.Vec3Int x1 x2 x3)      = CG.S.Vec3Int x1 x2 x3
dropStmt (LL.S.Vec4Int x1 x2 x3 x4)   = CG.S.Vec4Int x1 x2 x3 x4
dropStmt (LL.S.Vec2Bool x1 x2)        = CG.S.Vec2Bool x1 x2
dropStmt (LL.S.Vec3Bool x1 x2 x3)     = CG.S.Vec3Bool x1 x2 x3
dropStmt (LL.S.Vec4Bool x1 x2 x3 x4)  = CG.S.Vec4Bool x1 x2 x3 x4


dropStmt (LL.S.Lit val) = CG.S.Lit (Lit.dropLiteral val)
dropStmt (LL.S.Ref ident) = CG.S.Ref (ID.dropIdent ident)

dropStmt (LL.S.FunCall name args) =
    CG.S.FunCall
        (ID.dropIdent name)
        (map dropStmt args)
