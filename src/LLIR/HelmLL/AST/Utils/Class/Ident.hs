...
module LLIR.HelmLL.AST.Utils.Class.Ident (
    Identifiable(..)
  , gets
  , getText
  , getTexts
...
instance Identifiable Decl.Function where
    get (Decl.Function (Etc.Binder name _) _ _ _) = name


instance Identifiable Decl.Union where
    get (Decl.Union name _ _) = name

instance Identifiable Decl.Constr where
    get (Decl.Constr name _) = name
