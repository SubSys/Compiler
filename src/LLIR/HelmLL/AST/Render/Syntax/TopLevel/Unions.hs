...
module LLIR.HelmLL.AST.Render.Syntax.TopLevel.Unions (
    renderUnion
) where
...
renderConstructor :: Decl.Constr -> Doc
renderConstructor (Decl.Constr name args) =
    let name' = ID.renderIdent name
...
renderConstructors :: [Decl.Constr] -> Doc
