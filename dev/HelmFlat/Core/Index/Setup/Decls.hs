...
module HLIR.HelmFlat.Core.Index.Setup.Decls (
    setupDecls
) where
...
import qualified HLIR.HelmFlat.AST.Utils.Auxiliary.Binders             as Binder
-- import qualified HLIR.HelmFlat.AST.Utils.Auxiliary.Functions.Superposed

import qualified HLIR.HelmFlat.AST.Utils.Auxiliary.Special.Overloaded as OL
import qualified HLIR.HelmFlat.AST.Utils.Auxiliary.Special.Superposed as Super
...
type ToProcess = Decl.Function
type Ignore = Decl.Function


setupDecls :: [Decl.Function] -> ([ToProcess], [[Ignore]])
setupDecls decls =
    let
        (overloads, normals) = OL.separateOverloads decls
    in
        (normals, overloads)
