...
module Helm.Toolchain.Core.Build.Frontend.Driver (
      frontendMaster
    , toHelmIntro
) where
...
import qualified HLIR.HelmIntro.Data.Payload as HelmIntro.Payload
...
import qualified Helm.Toolchain.Core.Build.Frontend.Pipeline.Program as Program
import qualified Helm.Toolchain.Core.Build.Frontend.Pipeline.Module  as Module
-- *


frontendMaster :: [FilePath] -> IO (Either Report.BuildError Program.Program)
frontendMaster paths =
    Module.buildSources paths |> Program.aggregateSources



toHelmIntro :: IO (Either Report.BuildError Program.Program)
            -> IO (Either Report.BuildError HelmIntro.Payload.Program)
toHelmIntro upstream = do
    result <- upstream
