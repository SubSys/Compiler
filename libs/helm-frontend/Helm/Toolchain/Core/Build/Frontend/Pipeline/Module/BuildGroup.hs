...
module Helm.Toolchain.Core.Build.Frontend.Pipeline.Module.BuildGroup (
    buildGroup
) where
...
import qualified Helm.Toolchain.Core.Build.Frontend.Pipeline.Module.BuildSource as Build
-- *




buildGroup :: [Module.Module]
           -> Log.BuildLog
           -> [(BuildOrder.ModuleName, BuildOrder.ModuleImports)]
           -> IO (Either Text [Module.Module])
buildGroup deps buildLog moduleNames = do
    results <- M.mapM (Build.buildSource deps buildLog) moduleNames
    
    let valids   = Either.rights results
        invalids = Either.lefts results
    
    if null invalids then 
        return $ Right valids
