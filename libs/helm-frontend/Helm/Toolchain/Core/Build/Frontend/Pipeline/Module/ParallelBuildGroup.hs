...
-- | (TODO) Parallel Build Group
module Helm.Toolchain.Core.Build.Frontend.Pipeline.Module.ParallelBuildGroup (
    parallelBuildGroup
) where
...
import qualified Helm.Toolchain.Core.Build.Frontend.Pipeline.Module.BuildGroup as Build
-- *



-- TODO: ...
parallelBuildGroup :: [Module.Module]
                   -> Log.BuildLog
                   -> [[(BuildOrder.ModuleName, BuildOrder.ModuleImports)]]
                   -> IO (Either Text [Module.Module])

parallelBuildGroup deps buildLog [] = return $ Right deps

parallelBuildGroup deps buildLog (group:rest) = do
    result <- Build.buildGroup deps buildLog group 
    
    case result of
        Left err -> return $ Left err
        Right payloads ->
            parallelBuildGroup (deps ++ payloads) buildLog rest
