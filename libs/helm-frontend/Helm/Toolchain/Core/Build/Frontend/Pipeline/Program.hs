...
module Helm.Toolchain.Core.Build.Frontend.Pipeline.Program (
    aggregateSources
) where
...
aggregateSources :: IO (Either Report.BuildError [Module.Module])
                 -> IO (Either Report.BuildError Program.Program)
aggregateSources upstream = do
    result <- upstream
    
    case result of
        Left err -> return $ Left err
        Right payloads ->
            aggregateSources' payloads
...
aggregateSources' :: [Module.Module] -> IO (Either Report.BuildError Program.Program)
aggregateSources' payloads =
    return
        $ Right $ IUtil.aggregateModules payloads
