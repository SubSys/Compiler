...
module Helm.Toolchain.Core.Initialize.Program.Pipeline.BuildOrder.Parallelize (
    evalOrder2ParallelPaths
) where
...
-- ~ BuildOrder - Helpers
import qualified Helm.Toolchain.Core.Initialize.Program.Pipeline.BuildOrder.Parallelize.Helpers as Helper
-- *



-- |
-- Analyze Evaluation order, and compute possible parallel paths.
--
evalOrder2ParallelPaths :: [(Text, [Text])] -> [[(Text, [Text])]]
evalOrder2ParallelPaths =
    List.groupBy Helper.parallelPathsPred
