...
module Helm.Toolchain.Core.Initialize.Program.Pipeline.BuildOrder.Sequence (
    computeEvalOrder
) where
...
-- ~ BuildOrder - Helpers
import qualified Helm.Toolchain.Core.Initialize.Program.Pipeline.BuildOrder.Sequence.Helpers as Helper
-- *



-- | Determine Evaluation order
computeEvalOrder :: AG.Graph Text -> [(Text, [Text])]
computeEvalOrder graph =
    independantNodes ++ restOrder
    
    where
        -- Initial Data
        nodes = Helper.removeSudoFFINamespace $ Helper.subgroupNodes graph
        
        -- Mid Data
        independantNodes = List.filter Helper.noImports nodes
        dependentNodes = List.filter (not . Helper.noImports) nodes
        
        
        -- Result
        restOrder = List.sortBy Helper.depOrderPred dependentNodes
