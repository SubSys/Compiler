...
module Helm.Toolchain.Core.Initialize.Program.Pipeline.BuildOrder.Parallelize.Helpers where
...
-- *
-- | `evalOrder2ParallelPaths` - Helpers (TMP - TODO: Move to sub module...)
-- *
parallelPathsPred :: (Text, [Text]) -> (Text, [Text]) -> Bool
parallelPathsPred (node1, []) (node2, []) = True

parallelPathsPred (node1, deps1) (node2, deps2) =
    deps1 == deps2
