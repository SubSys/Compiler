...
module Helm.Toolchain.Core.Initialize.Header.Pipeline.Parse (
    parseHeader
) where
...
parseHeader :: FilePath -> IO (Either Report.BuildError Module.ModuleHeader)
parseHeader modulePath = do
    result <- Driver.runHeaderParser (Text.pack modulePath) (SIO.readFile modulePath)
