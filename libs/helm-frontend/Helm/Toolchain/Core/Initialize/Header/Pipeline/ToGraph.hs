...
module Helm.Toolchain.Core.Initialize.Header.Pipeline.ToGraph (
    header2Graph
) where
...
header2Graph ::  Module.ModuleHeader -> Either Report.BuildError (Log.BuildLog, AG.Graph Text)
header2Graph payload
    | null imports =
        Right (envLog, AG.vertex moduleName')
    
    | importsSelf moduleName' imports' =
        Left $ Report.ImportsSelf moduleName
    ...
    where
        envLog = Map.singleton moduleName payload
        
        node = AG.edges
            $ List.zip (List.repeat moduleName') imports'
        
        -- Initial Data
        moduleName = Module.moduleName payload
        modulePath = Module.modulePath payload
        exports = Module.exports payload
        imports = Module.imports payload
        
        -- Serialized Data
        moduleName' = serializeNS moduleName
        imports'    = map serializeImport imports

        
        -- Helpers
        serializeNS :: ID.Namespace -> Text
        serializeNS (ID.Namespace segs) =
            Text.concat
                $ punctuate (Text.pack ".") segs
        
        serializeImport :: Decl.ImportDecl -> Text
        serializeImport (Decl.ImportDecl name _ _) =
            serializeNS name
