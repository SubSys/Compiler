...
module SLIR.HelmSyntax.Core.Module.Normalize.Utils.Namespace (
    overrideNS
) where
...
-- | Override Namespace
-- NOTE:
-- * Will apply to all 'namespace constructors' within the given datum.
--
overrideNS :: (Data a, Typeable a) => ID.Namespace -> a -> a
overrideNS ns =
    Uni.transformBi f
    where
