...
module SLIR.HelmSyntax.Core.Module.TypeCheck.Data.TypeSystem (
      closeOver
    , instantiate
    , generalize
    , freshTSPair
...
closeOver = normalize . generalize TI.empty
...
generalize :: TI.Env -> T.Type -> T.Scheme
generalize env t  =
...
freshIdent = do
    ...
    where
        ...
        label :: Text -> Text
        label x =
            prefix `Text.append` x
        
        pack :: Text -> ID.Low
        pack x = ID.Low (label x) Nothing Nothing
...
normalizeScheme :: T.Scheme -> T.Scheme
normalizeScheme = normalize
