...
module SLIR.HelmSyntax.Core.Module.TypeCheck.Syntax.Expr (
    inferExpr
) where
...
import qualified SLIR.HelmSyntax.Core.Module.TypeCheck.Syntax.Patterns    as P
...
inferExprList :: (Decl.Function -> Sys.Syntax Decl.Function)
              -> [E.Expr]
              -> Sys.State ([E.Expr], [T.Type], TI.Env)
inferExprList f es =
