...
module SLIR.HelmSyntax.Core.Module.TypeCheck.Syntax.Patterns (
    inferCaseAlts
) where
...
import SLIR.HelmSyntax.Core.Module.TypeCheck.Syntax.Misc.Utils          (inferList, inferMaybe)

-- ~ Sub Infers
import qualified SLIR.HelmSyntax.Core.Module.TypeCheck.Syntax.Base.Values as V
-- *
...
inferPattern f (P.Wildcard metaOpt) = do
    ...
    -- *
    binder (P.Wildcard metaOpt) tv TI.empty
...
inferPatternList :: (E.Expr -> Sys.Syntax E.Expr)
              -> [P.Pattern]
              -> Sys.State ([P.Pattern], [T.Type], TI.Env)
inferPatternList f =
