...
module SLIR.HelmSyntax.Core.Program.SDD.Inference.Utils.Auxiliary (
      inferList
    , inferMaybe
) where
...
--- Local Prelude
import SLIR.HelmSyntax.Core.Program.SDD.Inference.Utils.Syntax.Base (enter)
...
inferList :: (a -> Sys.Syntax a) -> [a] -> Sys.Infer ([a], [T.Type], Env.ExprEnv)
inferList f []     = do
    env <- M.ask
