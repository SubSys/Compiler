...
module CGIR.Rust.Core.Indexing.Data.System.Referable where
...
referable :: ID.Low -> Sys.State (ID.Low, Sub.Subst)
referable (ID.Low txt ns) = do
    (_, s) <- M.ask

    case Sub.lookup txt s of
        Nothing   -> return (ID.Low txt  ns, Sub.empty)
        Just txt' -> return (ID.Low txt' ns, Sub.empty)
