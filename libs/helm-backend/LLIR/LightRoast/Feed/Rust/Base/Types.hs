...
module LLIR.LightRoast.Feed.Rust.Base.Types (
    dropType
) where
...
dropType (L.T.Record fields ) =
    R.T.Record $ map dropField fields

dropType (L.T.Tuple items ) =
    R.T.Tuple $ map dropType items

dropType (L.T.List ty ) =
    R.T.List $ dropType ty

dropType (L.T.Union name args ) =
    R.T.Union
        (ID.dropBig name)
        (map dropType args)

dropType (L.T.Generic (L.Etc.Generic name)) =
    R.T.Generic
        $ R.Etc.Generic
        $ ID.dropLow name

dropType (L.T.Fn inTypes (L.Etc.Output out) ) =
    R.T.Fn
        (map dropType inTypes)
        (R.Etc.Output $ dropType out)


dropType L.T.String = R.T.String
dropType L.T.Char = R.T.Char
dropType L.T.Int = R.T.Int
dropType L.T.Float = R.T.Float
dropType L.T.Bool = R.T.Bool
...
dropField :: (L.ID.Low, L.T.Type) -> (R.ID.Low, R.T.Type)
dropField (name, ty) =
    (ID.dropLow name, dropType ty)
