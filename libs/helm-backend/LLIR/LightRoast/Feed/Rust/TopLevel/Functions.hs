...
module LLIR.LightRoast.Feed.Rust.TopLevel.Functions (
    dropFunction
) where
...
import qualified LLIR.LightRoast.Feed.Rust.TermLevel.Stmt as S
import qualified LLIR.LightRoast.Feed.Rust.Base.Types     as T
-- *



dropFunction :: L.Decl.Function -> R.Decl.Function
dropFunction (L.Decl.Function name gs ins out body) =
    let name' = ID.dropLow name
        gs'   = map dropGeneric gs
        ins'  = map dropInput ins
        out'  = dropOutput out
        body' = S.dropBlock body
    in
        R.Decl.Function name' gs' ins' out' body'
    




dropGeneric :: L.Etc.Generic -> R.Etc.Generic
dropGeneric (L.Etc.Generic name) =
    R.Etc.Generic $ ID.dropLow name

dropInput :: L.Etc.Input -> R.Etc.Input
dropInput (L.Etc.Input name ty) =
    R.Etc.Input
        (ID.dropLow name)
        (T.dropType ty)

dropOutput :: L.Etc.Output -> R.Etc.Output
dropOutput (L.Etc.Output ty) =
    R.Etc.Output $ T.dropType ty
