...
module LLIR.LightRoast.Render.Instances where
...
renderFunction (Decl.Function name args expr (Just sig)) =
    ...
    in
             name' <+> ":" <+> sig'
        <$$> name' <+> args' <+> "=" <$$> Util.indent 4 expr' <> Util.linebreak

renderFunction (Decl.Function name args expr Nothing) =
    let name' = render name
        expr' = render expr
        args'  = map render args
            |> Util.punctuate Util.space
...
instance Render Etc.Arg where
    render (Etc.Arg binder Nothing) =
        render binder

    render (Etc.Arg binder (Just ty)) =
        let binder' = render binder
            ty'     = render ty
        in
            binder' <> ":" <> ty'
