...
module HLIR.HelmOutro.Core.Lift.Driver (
      lift
    , lift'
) where
...
import qualified HLIR.HelmOutro.Core.Lift.Syntax      as Syntax
-- *




lift :: IO (Either Text Payload.Module) -> IO (Either Text Payload.Module)
lift upstream = do
    result <- upstream
...
lift' :: Payload.Module -> Either Text Payload.Module
lift' payload =
    let fns = Payload.getFunctions payload
        env = Sys.initEnv fns
    in
        case Syntax.topLevels env fns of
            Left err -> Left $ Text.pack $ show err
            Right fns' ->
                Right
                    $ Payload.updateFunctions payload $ unwrap fns'
