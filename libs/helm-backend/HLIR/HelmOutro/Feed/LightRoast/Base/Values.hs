...
module HLIR.HelmOutro.Feed.LightRoast.Base.Values (
    dropValue
) where
...
dropValue :: O.V.LiteralValue -> L.V.LiteralValue
dropValue (O.V.Char val) =
    L.V.Char val

dropValue (O.V.String val) =
    L.V.String val

dropValue (O.V.Int val) =
    L.V.Int val

dropValue (O.V.Float val) =
    L.V.Float val

dropValue (O.V.Bool val) =
    L.V.Bool val
